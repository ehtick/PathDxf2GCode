...
public interface IRawSegment {
    ...
    public void AddTo(List<IRawSegment> result, HashSet<IRawSegment> traversed, ref Vector2 currEnd) {
        if (End.Near(currEnd)) {
            Reverse();
        }
        result.Add(this);
...
public class MillChain : PathSegment {
    ...
    public override Vector3 EmitGCode(Vector3 currPos, double h_mm, Transformation3 t,
        ...
        {
            double firstMillingBottomH_mm = h_mm + (_params!.W_mm ?? _params!.T_mm - _params!.I_mm);
            foreach (var s in _segments) {
                List<Edge> edgesOfS = new();
                Edge? prev = default;
                double bottomH_mm = h_mm + s.Bottom_mm;
                for (double millingBottom_mm = firstMillingBottomH_mm; millingBottom_mm.Ge(bottomH_mm); millingBottom_mm -= _params!.I_mm) {
                    edgesOfS.Add(prev = new Edge(s, millingBottom_mm, prev));
                }
                if (prev == null || !prev.MillingBottom_mm.Near(bottomH_mm)) {
                    edgesOfS.Add(new Edge(s, bottomH_mm, prev));
                }
...
public class SweepSegment : PathSegmentWithParamsText<SweepSegment.RawSegment, IParams>, ILeafSegment {
    public new class RawSegment : PathSegmentWithParamsText<RawSegment, IParams>.RawSegment, IRawSegment {
...
public class HelixSegment : MarkOrMillPathSegment<HelixSegment.RawSegment, HelixParams> {
    public new class RawSegment : MarkOrMillPathSegment<RawSegment, HelixParams>.RawSegment, IRawSegment {
...
public class DrillSegment : MarkOrMillPathSegment<DrillSegment.RawSegment, DrillParams> {
    public new class RawSegment : MarkOrMillPathSegment<RawSegment, DrillParams>.RawSegment, IRawSegment {
        public Vector2 Center { get; }
        // See HelixSegment
